...
var TextmateProvider = class extends BaseProvider {
    ...
    if (this.config.loadFromExtensions) {
      ...
      import_coc5.extensions.onDidLoadExtension((extension) => {
        this.loadSnippetDefinition(extension).then((items) => {
          if (items == null ? void 0 : items.length) {
            items = items.filter((o) => import_coc5.workspace.languageIds.has(o.languageId));
  ...
  async loadByLanguageId(languageId) {
    ...
    for (let languageId2 of filetypes) {
      ...
      for (let [extensionId, items] of this.definitions.entries()) {
        for (let item of items) {
          if (item.languageId !== languageId2)
            continue;
          let arr = await this.loadSnippetsFromFile(item.filepath, languageId2, extensionId);
          if (arr)
            snippets.push(...arr);
        }
      }
      loaded[languageId2] = snippets;
    }
  }
  async getSnippetFiles(filetype) {
    let filetypes = this.getFiletypes(filetype);
    ...
    for (let filetype2 of filetypes) {
      let snippets = this.loadedSnippets[filetype2];
  ...
  async getTriggerSnippets(document, position, autoTrigger) {
    ...
    for (let snip of snippets) {
      ...
      edits.push({
        ...
        priority: (_a = snip.priority) != null ? _a : 0
      });
    }
    return edits;
  }
  ...
  async loadSnippetsFromFile(snippetFilePath, languageId, extensionId) {
    let contents;
  ...
  loadSnippetsFromText(filepath, languageId, contents) {
    let snippets = [];
    try {
      let errors = [];
...
async function activate(context) {
  ...
  subscriptions.push(import_coc8.listManager.registerList(new SnippetsList(import_coc8.workspace.nvim, manager, mru)));
  return {
    expandable: async () => {
      let bufnr = await nvim.eval('bufnr("%")');
      let edits = await manager.getTriggerSnippets(bufnr);
